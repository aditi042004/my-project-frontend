...
export default function App() {
  ...
  const [quizMode, setQuizMode] = useState('normal'); // 'normal' or 'custom'

  // NEW: State for user progress, loaded from localStorage.
  ...
  // NEW: useEffect to save progress to localStorage whenever it changes.
  useEffect(() => {
    try {
  ...
  // Function to start a custom quiz from the dashboard
  ...
  const renderCurrentPage = () => {
    switch (currentPage) {
      ...
      case PAGES.CHATBOT:
        return <ChatbotPage />;
      // NEW: Case for rendering the Dashboard page.
...
const Header = ({ navigateTo }) => (
  <header className="flex flex-col sm:flex-row justify-between items-center pb-4 border-b-2 border-purple-500/30">
    ...
    <nav className="flex flex-wrap justify-center gap-x-4 gap-y-2 mt-4 sm:mt-0">
      ...
      {/* NEW: Navigation button for the Dashboard */}
...
const FileLoader = ({ forGame, setGameData, navigateToGame }) => {
    ...
    const handleUpload = async () => {
        ...
        try {
            // NEW: Updated API endpoint to use environment variable
...
// --- Game Page Component ---
// NEW: Heavily modified to handle normal vs. custom quizzes and track progress.
const GamePage = ({ allGameData, quizMode, setQuizMode, userProgress, setUserProgress }) => {
    ...
    useEffect(() => {
        if (quizMode === 'custom') {
            // Generate quiz from user's weak words
    ...
    const handleAnswer = (selectedWord) => {
        ...
        if (correct) {
            ...
            // NEW: Add the incorrect word to the user's progress for practice.
    ...
    const handleNext = () => {
        ...
        if (currentQuestionIndex < questions.length - 1) {
            ...
            setGameOver(true);
            // NEW: Save quiz result to history
            setUserProgress(prev => ({
                ...prev,
...
// NEW: All-new component for the User Dashboard.
